...
class MatchingTablesController(http.Controller):
    @http.route('/get_matching_tables', type='json', auth='user')
    def get_matching_tables(self, capacity=None, location_table=None, characteristic=None):
        # Xử lý yêu cầu từ client và trả về dữ liệu về các bàn phù hợp
        matching_tables = request.env['manager.table'].search([
            ('capacity', '=', capacity),
            ('location_table', '=', location_table),
            ('characteristic', '=', characteristic)
        ])

        # Chuyển đổi danh sách các bàn phù hợp thành một danh sách các ID để trả về cho client
        matching_table_ids = matching_tables.ids
...
class OnlineSubmitForm(http.Controller):
    ...
    def final_submission(self, **kw):
        ...
        if final_values:
            return request.render("mbs_online_appointment.thank_you_template_id", {'final_values': final_values})
        else:
            return request.render("mbs_online_appointment.thank_you_template_id", {'final_values': {}})
...
class ThanksForm(http.Controller):
    ...
    def thank_you(self, **kw):
        ...
        request.env['appointment.appointment'].create({
            'hospital': dr_data.get('hospital') or False,
            'dr_nm': dr_data.get('dr_nm') or False,
            'date': dr_data.get('date') or False,
            'time': dr_data.get('time') or False,
